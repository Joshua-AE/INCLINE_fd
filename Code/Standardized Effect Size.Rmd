...
# species with most medium to high effect sizes
SES_05 <- traits_f %>% select(siteID, species, trait, SES)
SES_05$threshold <- case_when(SES_05$SES >= 1 | SES_05$SES <= - 1 ~ 1)
SES_05 <- SES_05 %>% na.omit() %>% group_by(species) %>% summarize(sum_SES05 = sum(threshold), .groups = "drop")

# comparison alpine vs generalist species
classification <- read_tsv("../Input data/species classifications.txt", guess_max = Inf)
