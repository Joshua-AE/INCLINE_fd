...
# OLD LEAF CHEMICAL TRAIT DATASET
#chem_traits <- readr::read_tsv("../Input data/PFTC6_Incline_clean_chemical_traits_2022.txt", guess_max = Inf)

# New chem traits table has 131 more c_n, 147 more d15n, 131 more of c_percent and n_percent, no change in d13c and p_percent
chem_traits <- readr::read_tsv("../Input data/PFTC6_Incline_clean_chemical_traits_2022_new.txt", guess_max = Inf)
...
```
# 835 SES values, grouped by block and site, we might overestimate effect sizes due to low sample size per block. Number of paired observations more unbalanced based on blockID, much lower sample size for chem traits than leaf traits
...
#species_SES_table <- traits4 %>% filter(!is.na(SES)) %>% group_by(species, trait) %>%
#                     mutate(mean_SES = mean(SES)) %>% select(species, trait, mean_SES) %>% sample_n(1) %>% pivot_wider(names_from = trait, values_from = mean_SES)

test <- traits4 %>% filter(!is.na(SES)) %>% group_by(trait) %>% mutate(trait_mean = mean(SES), trait_sd = sd(SES))
test$outlier2 <- case_when(test$SES <= test$trait_mean - 2* test$trait_sd ~ "low", test$SES >= test$trait_mean + 2* test$trait_sd ~ "high")
test$outlier3 <- case_when(test$SES <= test$trait_mean - 3* test$trait_sd ~ "low", test$SES >= test$trait_mean + 3* test$trait_sd ~ "high")
test$`outlier2.5` <- case_when(test$SES <= test$trait_mean - 2.5* test$trait_sd ~ "low", test$SES >= test$trait_mean + 2.5* test$trait_sd ~ "high")
# 18 SES values removed with 2sd threshold for each trait. 13 removed for 2sd threshold overall and 3 removed for 3sd threshold overall. 6 for 2.5 sd overall

# First, removing outlier values that are further than 3 sd away from the mean. One D15N, one C% and one D13C observation
trait_SES_table <- traits4 %>% filter(SES <= 3 & SES >= -3) %>% group_by(trait) %>% mutate(meanSES = mean(SES), absSES = mean(abs(SES)), nsamples = n()) %>%
                   sample_n(1) %>% ungroup() %>% select(c(trait, meanSES, absSES, nsamples))

sp_tables <- traits4 %>% filter(!is.na(SES)) %>% select(siteID, species, trait, SES) #%>% filter(trait != "C/N")
...
table_plots <- ggplot(sp_tables, aes(x = precip, y = species, size = abs(SES), color = SES > 0)) + geom_point() +
  ...
  scale_size_continuous(range = c(0.5,7), breaks = c(0.1, 1, 2, 3)) + theme_classic() + # c(1, 10)
  ...
  facet_wrap(~ trait, scales = "free", ncol = 3) + annotate("text", x = 4.3, y = 8, label = "alpine", angle = 90, size = 4) + # before: x = 4.4, y = 7.5
...
```{r}
SES_sites <- traits4 %>% select(siteID, species, trait, SES) %>% na.omit()
...
SES_sites <- SES_sites %>% filter(SES <= 3 & SES >= -3) # removing outlier values that are further than 3 sd away from the mean. One D15N, one C% and one D13C observation
