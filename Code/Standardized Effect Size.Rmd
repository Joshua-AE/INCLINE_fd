...
# Instead, use e.g. MAD mean absolute deviation. 1/n * sum(abs(x-mean))
...
species_sd <- species_sd %>% group_by(species) %>% summarise(mean_sd = mean(rel_sd, na.rm = T)) %>%
  left_join(species_n, by = "species") %>% left_join(distribution, by = "species") %>% left_join(species_abs_SES, by = "species")
