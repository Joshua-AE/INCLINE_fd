...
multi_bp <- ggplot(SES_sites, aes(x = trait, y = SES, group = trait)) + geom_boxplot(aes(fill = trait), outlier.alpha = 0) +
  theme_classic() + geom_hline(yintercept = 0, lty = 2, size = 0.25) +
  geom_jitter(color = "black", height = NULL, width = 0.3, alpha = 0.5) + ylim(-2.5,2)
multi_bp # add points in background with alpha
