...
new_chem_traits <- readr::read_tsv("../Input data/PFTC6_Incline_clean_chemical_traits_2022_new.txt", guess_max = Inf)
...
traits3 <- cbind(traits1, traits1l$n_obs, traits1sd$sd)
traits3 <- rename(traits3, n_obs = `traits1l$n_obs`, sd = `traits1sd$sd`)
traits3$ID <- paste(traits3$siteID, traits3$blockID, traits3$species, traits3$trait, sep = "_")
...
traits3OTC <- traits3OTC %>% rename(OTC_mean = mean_value, OTC_n = n_obs, OTC_sd = sd) %>%
  select(-c(siteID, blockID, species, warming, trait))
...
SES_sites <- traits4 %>% select(c(1,3:4,15)) %>% na.omit()
...
panel_plots <- ggplot(SES_combined, aes(x = siteID, y = SES, fill = siteID)) +
  ...
  ylim(-5,5) + # otherwise ylim(-2.5,2.5)
