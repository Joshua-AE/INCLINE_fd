...
# AT THIS POINT, DECISION ON DUPLICATE VALUES HAS TO BE MADE: Currently, summarizing duplicate values into one mean per ID/ merged_ID
chem_traits <- chem_traits %>% group_by(ID_merged, trait) %>% mutate(mean_value = mean(value)) %>% sample_n(1) 
chem_traits <- chem_traits %>% ungroup() %>% select(c(siteID, blockID, warming, species, trait, mean_value)) %>% na.omit() %>%
...
chem_traits <- rename(chem_traits, value = mean_value)
