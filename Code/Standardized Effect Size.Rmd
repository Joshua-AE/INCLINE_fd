...
traits3OTC <- traits3OTC %>% rename(OTC_mean = mean_value, OTC_n = n_obs, OTC_sd = sd) %>%
  select(-c(siteID, traits3$blockID, species, warming, trait))
...
```
# 835 SES values, grouped by block and site, 
...
multi_bp <- ggplot(SES_sites, aes(x = trait, y = SES, group = trait)) + geom_violin(aes(fill = trait), trim = T) +
  theme_classic() + geom_hline(yintercept = 0, lty = 2, linewidth = 0.25) + ylim(-2.5,2) + stat_summary(fun.data = "mean_cl_boot", geom = "pointrange",
               colour = "black")
# + geom_boxplot(aes(fill = trait), outlier.alpha = 0)
# + geom_jitter(color = "black", height = NULL, width = 0.3, alpha = 0.5)
