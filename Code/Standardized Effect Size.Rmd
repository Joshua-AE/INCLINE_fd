...
height_table <- traits4 %>% filter(!is.na(SES) & trait == "plant_height_cm") %>% select(siteID, species, SES) %>%
                pivot_wider(names_from = siteID, values_from = SES) %>% relocate(species, Ulvehaugen, Gudmedalen, Skjelingahaugen)
...
#_______________________________________________________________________________ violin plots in panels, global vs each site
data_all_sites <- SES_sites %>% mutate(siteID = "All Sites")
SES_combined <- bind_rows(SES_sites, data_all_sites)
SES_combined <- SES_combined %>% mutate(siteID = factor(siteID, levels = c("All Sites", "Ulvehaugen", "Gudmedalen", "Skjelingahaugen")))

panel_plots <- ggplot(SES_combined, aes(x = siteID, y = SES, fill = siteID)) +
  ...
  theme_classic() +
  labs(
    title = "Effect of warming on intraspecific trait values",
    x = "Site, precipitation 1300 mm, 2100 mm, 3600 mm",
