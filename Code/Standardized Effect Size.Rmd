...
sp_tables <- traits4 %>% filter(!is.na(SES)) %>% select(siteID, species, trait, SES) 
...
table_plots <- ggplot(sp_tables, aes(x = precip, y = species, size = abs(SES), color = SES > 0)) + geom_point() +
  ...
  scale_size_continuous(range = c(1,10), breaks = c(0.1, 1, 2, 3)) + theme_classic() + # c(1, 4, 7, 10)
  ...
  facet_wrap(~ trait, scales = "free", ncol = 5) + annotate("text", x = 4.4, y = 7.5, label = "alpine", angle = 90, size = 4) +
  annotate("text", x = 4.4, y = 2, label = "generalist", angle = 90, size = 4)
