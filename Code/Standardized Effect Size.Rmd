...
traits_n13 <- traits %>% filter(species %in% selected_sp) # trait set with only 13 selected species. 37.3 % of data omitted
...
traits1 <- aggregate(traits_n13$value, by = traits_n13[,1:5], FUN = na.omit(mean))
traits1 <- rename(traits1, mean_value = x)
traits1l <- aggregate(traits_n13$value, by = traits_n13[,1:5], FUN = na.omit(length))
traits1l <- rename(traits1l, n_obs = x)
traits1sd <- aggregate(traits_n13$value, by = traits_n13[,1:5], FUN = na.omit(var)) # default: sample not population variance
...
traits2 <- aggregate(traits_n13$value, by = traits_n13[,c(1,3:5)], FUN = na.omit(mean))
traits2 <- rename(traits2, mean_value = x)
traits2l <- aggregate(traits_n13$value, by = traits_n13[,c(1,3:5)], FUN = na.omit(length))
traits2l <- rename(traits2l, n_obs = x)
traits2sd <- aggregate(traits_n13$value, by = traits_n13[,c(1,3:5)], FUN = na.omit(var)) # default: sample not population variance
