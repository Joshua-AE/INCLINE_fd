...
trait_all <- trait_all %>% select(-c(warming, Freq, species, siteID)) %>% column_to_rownames(var = "speciesID")
...
out <- list() # DOESNT WORK YET> I PROBABLY HAVE TO SUPPLY EACH SITE ONE AFTER THE OTHER AN NOT ALL AT ONCE
for(i in 1:10) {
  set.seed(0+i)
  print(i)
hvlist = kernel.build(comm = comm_all, trait = trait_pca, distance = "euclidean", method.hv = "gaussian",
                             abund = FALSE, weight = NULL, axes = 0, convert = 0)
