...
# 10 traits are too much. We have to separat growth and leaf traits from chemical traits
```{r}
trait_i <- traits %>% filter(trait %in% c("plant_height_cm", "leaf_area_cm2", "leaf_thickness_mm", "ldmc", "sla_cm2_g")) %>% group_by(siteID, blockID, warming, species, trait) %>% summarise(mean_value = mean(value, na.rm = TRUE)) %>% pivot_wider(names_from = trait, values_from = mean_value) %>% na.omit() 
# "c_n", "c_percent", "d15n", "n_percent", "d13c"

# Optional: Select different sites and create PCAs for each site
trait_i <- trait_i %>% filter(siteID == "Skjelingahaugen") %>% ungroup() # precipitation: Ulvehaugen < Gudmedalen < Skjelingahaugen

pairs.panels(trait_i[,c(5:9)], smooth = TRUE, ellipse = FALSE, method = "pearson", hist.col = 4, stars = TRUE, cex = 1)
# most correlated with other traits are: cn > sla > ldmc > h
# sqrt() or log10() for traits that are right-skewed (positive skew)
trait_i[,c(6:9)] <- log10(trait_i[,c(6:9)]) # leaf area, leaf thickness, plant height, c_n, sla(?) sqrt transform

pca1 <- prcomp(trait_i[5:9], center = T, scale = T)
pcomp1 <- as.data.frame(pca1$rotation)
summary(pca1)

fviz_pca_biplot(pca1, axes = c(1,2), geom = "point", repel = TRUE, col.var = "#2E9FDF", col.ind = "#696969", xlim = c(-5,5), ylim = c(-5,5), habillage = trait_i$warming,
             addEllipses = TRUE, ellipse.level = 0.95) + scale_color_manual(values=c("#2E9FDF", "red")) + scale_fill_manual(values=c("#2E9FDF", "red")) +
             labs(title = "PCA")
# print 600 x 400
