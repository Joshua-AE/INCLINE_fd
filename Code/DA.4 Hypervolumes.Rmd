...
for(i in 1:100) {
  set.seed(0+i)
  print(i)
  hp <- kernel.build(comm = comm_all, trait = trait_i, distance = "euclidean", method.hv = "gaussian",
...
plan(multisession)
ptm <- proc.time()
fric_all_2 <- foreach(i = 1:100, .combine = rbind, .options.future = list(seed = TRUE)) %dofuture% {
set.seed(0+i)
kernel.alpha(kernel.build(comm = comm_all, trait = trait_all, distance = "euclidean", method.hv = "gaussian",
                             abund = FALSE, weight = NULL, axes = 0, convert = 0))
}
